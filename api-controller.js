...
module.exports = {
  ...
  ftn: function(msg, args){
    ...
    const options = {
      ...
      'headers': {
        'TRN-Api-Key': "3bc7150c-48db-4ff2-a764-713c8bf5c9c8",
    ...
    request(options, (err, res, body) => {
      if(err){
        console.error(err)
        msg.reply(`Il y a eu un problème en prennant les information de ${username.replace('%20', ' ')} sur ${platform}.`)
        return null
      }
        ...
        if(data.stats.p2){
          ...
          soloWin = data.stats.p2.top1matches
        }
    ...
    //    gWins = (Math.round((soloWins * 1)+ duoWins * 1)+ squadWins * 1);
     //   gKill = (Math.round((soloKill * 1)+ duoKill * 1)+ squadKill * 1);
